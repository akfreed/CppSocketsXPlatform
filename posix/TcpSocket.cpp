...
void TcpSocket::move(TcpSocket&& other) noexcept(false)
{
    ...
    memset(&other.m_hostInfo, 0, sizeof(other.m_hostInfo));
...
bool TcpSocket::Connect(const char* host, const char* port)
{
    ...
    // connect
    error = connect(m_socketId, m_hostInfoList->ai_addr, m_hostInfoList->ai_addrlen);
...
bool TcpSocket::Write(const char* buf, int len)
{
    ...
    ssize_t amountWritten = send(m_socketId, buf, len, MSG_NOSIGNAL);
...
bool TcpSocket::Read(char* dest, int len)
{
    // sanity check
    if (len < 1)
    {
    ...
    ssize_t amountRead = recv(m_socketId, dest, len, MSG_WAITALL);  // MSG_WAITALL blocks until all bytes are there
...
int TcpSocket::ReadString(char* buf, int maxlen)
{
    if (maxlen < 1)
    {
        assert(false);
...
bool TcpSocket::DumpReadBuffer()
{
    ...
    while ((available = DataAvailable()) > 0)
    {
        ...
        amountRead = recv(m_socketId, buffer, (available < BUFFER_SIZE ? available : BUFFER_SIZE), 0);
